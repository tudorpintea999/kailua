...
/// @notice Thrown when a resolution is attempted against an unproven challenged game
error NotYetProven();
...
contract FaultClaimGame is Clone, IDisputeGame {
    ...
    /// @notice Returns the dispute game factory contract.
    ...
    function initialize() external payable {
        ...
        uint64 parentBlockNumber = parentGame().l2BlockNumber();
    ...
    function resolve() external returns (GameStatus status_) {
        ...
        if (challenger != address(0x0)) {
            ...
            if (proofStatus == ProofStatus.FAULT) {
                ...
                // INVARIANT: Cannot resolve an unproven challenged game
                revert NotYetProven();
            }
        } else {
    ...
    function l2BlockNumber() public pure returns (uint64 l2BlockNumber_) {
    ...
    function parentGame() public view returns (FaultClaimGame parentGame_) {
        ...
        parentGame_ = FaultClaimGame(address(parentDisputeGame));
    }
